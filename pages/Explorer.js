...
export const Explorer = ({ mode, isAppMode, uiConfig }) => {
  ...
  const renderMainContent = () => {
    ...
    return html`
      <header key="explorer-header" className=${`mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 ${isAppMode ? `p-6 rounded-full border mx-4 mt-6 ${isLiquid ? 'bg-white/40 backdrop-blur-sm border-white/20 shadow-sm' : 'bg-white border-slate-200 shadow-sm'}` : 'px-2'}`}>
